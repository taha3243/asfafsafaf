...
exports.run = async (client, message, args) => {
  if (
    !["779032096411746315", "779032079727591484", "779032078351597637"].some(
  ...
  const kayıtlı = message.guild.roles.cache.find(
    r => r.id === "779032117380251659"
  );
  const kayıtsız = message.guild.roles.cache.find(
    r => r.id === "779032118709321759"
  );
  ...
  const embed = new Discord.MessageEmbed()
    ...
    .addField(
      `<a:fors34:778005711509913621> Kayıt Eden:`,
      `<@${message.author.id}> Tarafından Kayıt Edildi`
    )
    .addField(
      `<a:fors34:778005711509913621> Kayıt Edilen:`,
      `<@${stg.user.id}> Kayıt Oldu`
    )
    .addField(
      `<a:fors35:778005712876863548> Verilen Rol:`,
      `<@&${kayıtlı.id}> Rolleri Verildi`
    )
    .addField(
      `<a:fors37:778005711354462209> Alınan Rol:`,
      `<@&${kayıtsız.id}> Rolleri Alındı`
    )
    .addField(
      `<a:fors33:778005711442804766> Yeni İsmin:`,
      `\`${tag} ${isim} | ${yas}\` Olarak Güncellendi`
    )
    .addField(
      `<a:fors33:778005711442804766> Yetkili Toplam:`,
